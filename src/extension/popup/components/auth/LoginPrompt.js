...
const LoginPrompt = () => {
    const { login, isLoading, error, clearError } = (0, AuthContext_1.useAuthContext)();
    return (<div className="flex flex-col items-center justify-center h-96 p-6">
      ...
      <Button_1.Button onClick={login} isLoading={isLoading} className="flex items-center gap-3 px-6 py-3" disabled={isLoading}>
        <GoogleIcon />
        {isLoading ? 'Signing in...' : 'Sign in with Google'}
      </Button_1.Button>
      
      {error && (<ErrorMessage_1.ErrorMessage message={error} className="mt-4 text-center max-w-sm" onRetry={clearError}/>)}
