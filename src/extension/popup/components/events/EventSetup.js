...
const EventSetup = ({ onEventConnected, onCancel }) => {
    const [sheetId, setSheetId] = (0, react_1.useState)('');
        ...
        try {
            ...
            if (response.success) {
                ...
                if (response.error?.includes('README tab not found')) {
                    setError('README tab not found in your Google Sheets. Please create a README tab with a table containing columns: Tab, Header Row, Column, Column Description. This helps EventConnect understand your sheet structure.');
                }
                else {
    ...
    return (<div className="bg-white border border-gray-200 rounded-lg p-4">
      ...
      <div className="space-y-4">
        <div>
          ...
          <input id="sheetId" type="text" value={sheetId} onChange={(e) => setSheetId(e.target.value)} placeholder="https://docs.google.com/spreadsheets/d/YOUR_SHEET_ID/edit or just the Sheet ID" className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" disabled={isConnecting}/>
          <p className="text-xs text-gray-500 mt-1">
            Paste the full URL or just the Sheet ID (found between /d/ and /edit in the URL)
          </p>
        </div>

        {error && (<div className="flex items-center gap-2 text-red-600 bg-red-50 p-3 rounded-lg">
            <outline_1.ExclamationCircleIcon className="w-5 h-5 flex-shrink-0"/>
            <span className="text-sm">{error}</span>
          </div>)}

        <div className="flex gap-2">
          <button onClick={connectSheet} disabled={!sheetId.trim() || isConnecting} className="flex-1 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg 
          ...
          <button onClick={onCancel} disabled={isConnecting} className="px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 
        ...
        <div className="border-t pt-4">
          ...
          <button onClick={openSheetCreationHelp} className="text-blue-600 hover:text-blue-800 text-sm font-medium">
