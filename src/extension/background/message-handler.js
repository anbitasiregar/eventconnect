...
class MessageHandler {
    constructor(authService, apiClient, sheetsService) {
        ...
        this.RATE_LIMIT_MAX_REQUESTS = 60; // 60 requests per minute
    ...
    async handleMessage(message, sender) {
        try {
            logger_1.Logger.debug(`Handling message: ${message.type}`, {
            ...
            if (cacheKey && this.isGetOperation(message.type)) {
                ...
                if (cached) {
                    logger_1.Logger.debug(`Returning cached response for: ${message.type}`);
                    return cached;
                }
            }
            let response;
            ...
            logger_1.Logger.debug(`Message handled successfully: ${message.type}`, {
                requestId: message.requestId
            });
            return response;
        }
        catch (error) {
            logger_1.Logger.error(`Message handling failed: ${message.type}`, error, {
                requestId: message.requestId
            });
            // Return user-friendly error message
            const errorMessage = this.getUserFriendlyError(error, message.type);
   ...
   */
                ...
                try {
                    ...
                    if (storedEventId.currentEventId) {
                        ...
                        const localEvent = {
                            ...
                            status: 'in_progress' // Could be enhanced later
                        };
     ...
     */
    async handleSheetsMessage(message) {
        switch (message.type) {
    ...
    async handleActionMessage(message) {
        switch (message.type) {
            case 'EXECUTE_ACTION':
                ...
                logger_1.Logger.info(`Action execution requested: ${message.payload.action}`);
                return {
                    success: true,
    ...
    clearCache() {
        ...
        logger_1.Logger.debug('Response cache cleared');
    }
    /**
    ...
    clearEventCache() {
        ...
        logger_1.Logger.debug('Event cache cleared');
    }
    /**
     * Clear sheets-specific cache
     */
    clearSheetsCache(sheetId) {
        for (const key of this.responseCache.keys()) {
            if (key.includes(`"sheetId":"${sheetId}"`)) {
                this.responseCache.delete(key);
            }
        }
        logger_1.Logger.debug(`Sheets cache cleared for: ${sheetId}`);
    }
    /**
    ...
    checkRateLimit(sender) {
        ...
        if (senderData.count > this.RATE_LIMIT_MAX_REQUESTS) {
            logger_1.Logger.warn(`Rate limit exceeded for sender: ${senderId}`);
            return false;
        }
        return true;
    }
     ...
     */
    getUserFriendlyError(error, messageType) {
