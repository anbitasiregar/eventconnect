...
class GoogleAuthService {
     ...
     */
    enrichTokenWithExpiration(token) {
        const now = Date.now();
    ...
    async storeToken(token) {
        ...
        await (0, storage_1.setStorageItem)('authToken', token);
    }
    /**
     * Retrieve stored token from Chrome storage
     */
    async getStoredToken() {
        return await (0, storage_1.getStorageItem)('authToken');
    }
    /**
...
GoogleAuthService.TOKEN_REFRESH_BUFFER = 5 * 60 * 1000; // 5 minutes in ms
